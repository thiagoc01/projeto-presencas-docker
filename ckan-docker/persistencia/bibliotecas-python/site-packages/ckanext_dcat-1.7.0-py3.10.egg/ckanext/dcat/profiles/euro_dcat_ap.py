...
class EuropeanDCATAPProfile(RDFProfile):
    ...
    def parse_dataset(self, dataset_dict, dataset_ref):
        ...
        for distribution in self._distributions(dataset_ref):
            ...
            geo_spatial_object = self._object_value_list(distribution, GSP.SpatialObject)

            for key, predicate in (
                ("hasMetricLength", GSP.hasMetricLength),
                ("hasArea", GSP.hasArea)
            ):
                
                value = self._object_value(geo_spatial_object, predicate)
    ...
    def graph_from_dataset(self, dataset_dict, dataset_ref):
        ...
        if self._get_dataset_value(dataset_dict, "first_name"):
            ...
            items = [
                ("first_name", FOAF.firstName, None, Literal),
                ("last_name",  FOAF.lastName, None, Literal),
                ("birthday", FOAF.birthday, None, Literal),
                ("homepage", FOAF.homepage, None, URIRefOrLiteral),
                ("gender", FOAF.gender, None, Literal)
            ]

            self._add_triples_from_dict(dataset_dict, person, items)

            g.add((dataset_ref, FOAF.Person, person))
        ...
        if person != None:
            ...
            items = [
                ("links", FOAF.page, None, URIRefOrLiteral)
            ]

            self._add_list_triples_from_dict(dataset_dict, person, items)
        ...
        for resource_dict in dataset_dict.get("resources", []):
            ...
            if resource_dict.get("hasMetricLength") or resource_dict.get("hasArea"):

                spatial_object = BNode()

                g.add((spatial_object, RDF.type, GSP.SpatialObject))

                if resource_dict.get("hasMetricLength"):
                    ...
                    try:
                        g.add(
                            (
                                spatial_object,
                                GSP.hasMetricLength,
                                Literal(float(resource_dict.get("hasMetricLength")), datatype=XSD.double),
                            )
                        )
                    except (ValueError, TypeError):
                        g.add(
                            (spatial_object, GSP.hasMetricLength, Literal(resource_dict.get("hasMetricLength")))
                        )

                elif resource_dict.get("hasArea"):
                    g.add(
                            (spatial_object, GSP.hasArea, Literal(resource_dict.get("hasArea")))
                    )

                g.add((distribution, GSP.SpatialObject, spatial_object))
